...
class TrafficSignalHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        elif self.path == '/esp32-data':
            # Create a simplified response for ESP32 with less data
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            
            # Simplified JSON with just what ESP32 needs
            response = {
                "signals": [
                    {"red": 1 if currentGreen != 0 else 0, 
                     "yellow": 1 if currentGreen == 0 and currentYellow == 1 else 0, 
                ...
                "accident": 1 if accidentOccurred else 0,
                "accidentDirection": accidentDirection if accidentOccurred else -1
            }
...
def start_server():
    class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
        def do_GET(self):
            if self.path == '/esp32-data':
                self.send_response(200)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                
                # Tạo dữ liệu JSON cho ESP8266
                data = {
                    "signals": [
                        {"red": 1 if signals[0].red > 0 else 0, "yellow": 1 if signals[0].yellow > 0 else 0, "green": 1 if signals[0].green > 0 else 0},
                        {"red": 1 if signals[1].red > 0 else 0, "yellow": 1 if signals[1].yellow > 0 else 0, "green": 1 if signals[1].green > 0 else 0},
                        {"red": 1 if signals[2].red > 0 else 0, "yellow": 1 if signals[2].yellow > 0 else 0, "green": 1 if signals[2].green > 0 else 0},
                        {"red": 1 if signals[3].red > 0 else 0, "yellow": 1 if signals[3].yellow > 0 else 0, "green": 1 if signals[3].green > 0 else 0}
                    ],
                ...
                print("ESP32 data requested. Sent signal states.")
            else:
                self.send_response(404)
                self.end_headers()
